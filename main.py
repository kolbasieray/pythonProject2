...
def baslik_ekle():
    if request.method == 'POST':
        ...
        db["basliklar"].insert_one({
            "_id": get_sequence("basliklar"),
            "baslik_id": int(baslik_id),
            "baslik": request.form["yeni_baslik"]
        })
        x = db["basliklar"].find()
        for document in x:
            print(document)
...
@app.route('/baslik-sil/<int:baslik_id>', methods=["POST"])
def baslik_sil(baslik_id):
    if request.method == 'POST':
        # Belirtilen baslik_id'ye sahip başlığı silme işlemi
        db["basliklar"].delete_one({"baslik_id": baslik_id})
